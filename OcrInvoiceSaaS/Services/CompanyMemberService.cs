...
public class CompanyMemberService : ICompanyMemberService
{
    ...
    public CompanyMemberService(ApplicationDbContext db)
    {
        _db = db;
    }
    ...
    public async Task<ServiceResult<List<CompanyMemberResponse>>> GetMembersAsync(Guid companyId, Guid userId)
    {
        ...
        if (!isMember)
            return ServiceResult<List<CompanyMemberResponse>>.Fail("Access denied.", 403);
        ...
        Guid companyId, InviteMemberRequest request, Guid requestingUserId)
    {
        ...
        if (!isOwnerOrAdmin)
            return ServiceResult<CompanyMemberResponse>.Fail("Only owners or admins can add members.", 403);

        // Only owners may assign Owner role
        ...
        if (targetUser == null)
            return ServiceResult<CompanyMemberResponse>.Fail(
                "No active account found with that email address.", 404);
    ...
    public async Task<ServiceResult> RemoveMemberAsync(
        Guid companyId, Guid targetUserId, Guid requestingUserId)
    {
        bool isOwnerOrAdmin = await _db.CompanyUsers.AnyAsync(cu =>
            cu.CompanyId == companyId &&
            cu.UserId == requestingUserId &&
        ...
        if (!isOwnerOrAdmin)
            return ServiceResult.Fail("Only owners or admins can remove members.", 403);

        // Prevent self-removal of owner
        if (targetUserId == requestingUserId)
            return ServiceResult.Fail("You cannot remove yourself from the company.", 400);

        var target = await _db.CompanyUsers
            .FirstOrDefaultAsync(cu => cu.CompanyId == companyId && cu.UserId == targetUserId);

        if (target == null)
            return ServiceResult.Fail("Member not found.", 404);
        ...
        if (target.Role == "Owner")
        {
            ...
            if (!requestingIsOwner)
                return ServiceResult.Fail("Admins cannot remove owners.", 403);
        }
