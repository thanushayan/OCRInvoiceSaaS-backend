...
public class LoginThrottleService : ILoginThrottleService
{
    ...
    // Configurable via appsettings.json → Security:Lockout
    ...
    public LoginThrottleService(ApplicationDbContext db, IConfiguration config)
    {
        _db = db;
        _config = config;
    }
    ...
    public async Task<ServiceResult<AccountStatusResponse>> CheckAsync(string email)
    {
        ...
        // Check hard lockout on user record first (persisted across restarts)
        if (user != null && user.IsLockedOut)
        {
            return ServiceResult<AccountStatusResponse>.Fail(
                $"Account is locked. Try again after {user.LockoutUntil:HH:mm UTC}.", 429);
        }

        // Count failed attempts within the rolling window
        ...
        var remaining = Math.Max(0, MaxFailedAttempts - recentFailures);

        if (recentFailures >= MaxFailedAttempts)
        {
            // Lock the account record too so it survives a window rollover
            if (user != null)
            {
            ...
            return ServiceResult<AccountStatusResponse>.Fail(
                $"Too many failed attempts. Account locked for {LockoutMinutes} minutes.", 429);
        }
    ...
    public async Task RecordSuccessAsync(string email, string? ipAddress)
    {
        ...
        // Clear failed counter on successful login
        var user = await _db.Users.FirstOrDefaultAsync(u => u.Email == normalised);
        if (user != null)
        {
