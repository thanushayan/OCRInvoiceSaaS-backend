...
public class ExpenseCategoryService : IExpenseCategoryService
{
    ...
    public ExpenseCategoryService(ApplicationDbContext db, IMapper mapper)
    {
        _db = db;
        _mapper = mapper;
    }
        ...
        Guid companyId, CreateExpenseCategoryRequest request, Guid userId)
    {
        ...
        return ServiceResult<ExpenseCategoryResponse>.Success(
            _mapper.Map<ExpenseCategoryResponse>(category), 201);
    }
    ...
    public async Task<ServiceResult<List<ExpenseCategoryResponse>>> GetByCompanyAsync(Guid companyId, Guid userId)
    {
        ...
        return ServiceResult<List<ExpenseCategoryResponse>>.Success(
            _mapper.Map<List<ExpenseCategoryResponse>>(categories));
    }
    ...
    public async Task<ServiceResult> DeleteAsync(Guid categoryId, Guid userId)
    {
        ...
        if (category == null)
            return ServiceResult.Fail("Category not found.", 404);

        if (!await UserBelongsToCompanyAsync(category.CompanyId, userId))
