...
public class InvoiceServiceV2 : IInvoiceService
{
    ...
    public InvoiceServiceV2(ApplicationDbContext db, IMapper mapper)
    {
        _db = db;
        _mapper = mapper;
    }
    ...
    public async Task<ServiceResult<InvoiceResponse>> CreateInvoiceAsync(
        Guid companyId, CreateInvoiceRequest request, Guid userId)
    {
    ...
    // Overload that accepts pagination query — used by the new controller
        ...
        Guid companyId, Guid userId, PaginationQuery query)
    {
        ...
        // Search across filename, invoice number, vendor name
        ...
        q = (query.SortBy?.ToLower(), query.SortDir?.ToLower()) switch
        {
            ...
            ("status", _) => q.OrderByDescending(i => i.Status),
    ...
    public async Task<ServiceResult<List<InvoiceListResponse>>> GetInvoicesAsync(
    ...
    public async Task<ServiceResult<InvoiceResponse>> GetInvoiceByIdAsync(Guid invoiceId, Guid userId)
    {
        ...
        if (invoice == null)
            return ServiceResult<InvoiceResponse>.Fail("Invoice not found.", 404);
    ...
    public async Task<ServiceResult<InvoiceResponse>> UpdateInvoiceAsync(
        Guid invoiceId, UpdateInvoiceRequest request, Guid userId)
    {
