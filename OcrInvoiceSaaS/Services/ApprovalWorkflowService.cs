...
public class ApprovalWorkflowService : IApprovalWorkflowService
{
        ...
        Guid companyId, CreateWorkflowTemplateRequest request, Guid userId)
    {
        ...
        // Validate step orders are unique and start at 1
        ...
        Guid invoiceId, StartApprovalRequest request, Guid userId)
    {
        ...
        // Block if already has active approval
        ...
        // Fire dispatcher — sends email + webhook
    ...
    public async Task<ServiceResult<List<ApprovalInstanceResponse>>> GetPendingForUserAsync(Guid userId)
    {
        ...
        // Get in-progress instances where the current step is assigned to this user
        ...
        // Filter to only instances where the current step belongs to this user
        ...
        Guid instanceId, SubmitApprovalActionRequest request, Guid userId)
    {
        ...
        if (action == ApprovalStepStatus.Rejected)
        {
            ...
            instance.Invoice.Status   = InvoiceStatus.Processed; // send back for review
    ...
    public async Task EscalateTimedOutStepsAsync()
    {
        ...
        foreach (var instance in instances)
        {
            ...
            if (step.IsOptional)
            {
                // Auto-skip optional timed-out steps
                _db.InvoiceApprovalActions.Add(new InvoiceApprovalAction
                {
            ...
            {
                // Notify initiator that step has timed out
    ...
    private async Task NotifyApproverAsync(InvoiceApprovalInstance instance, ApprovalWorkflowStep step)
    {
        if (step.AssignedUserId.HasValue)
        {
            await _notifications.CreateAsync(
                ...
                $"Step '{step.StepName}' requires your approval.",
                "Info");
        }
    }
