...
public class RateLimitService : IRateLimitService
{
    ...
    public RateLimitService(ApplicationDbContext db, IConfiguration config) { _db = db; _config = config; }

    public async Task<(bool allowed, RateLimitStatusResponse status)> CheckAndIncrementAsync(Guid companyId, string resource = "api")
    {
        var limit = await GetLimitForCompanyAsync(companyId);
        var windowStart = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, DateTime.UtcNow.Day, DateTime.UtcNow.Hour, 0, 0, DateTimeKind.Utc);
        var windowEnd = windowStart.AddHours(1);
        var windowKey = $"{windowStart:yyyyMMddHH}:{resource}";

        var record = await _db.ApiUsageRecords.FirstOrDefaultAsync(r => r.CompanyId == companyId && r.WindowKey == windowKey);
        if (record == null)
        {
            record = new ApiUsageRecord { CompanyId = companyId, WindowKey = windowKey, RequestCount = 0, WindowStart = windowStart, WindowEnd = windowEnd };
            _db.ApiUsageRecords.Add(record);
        }

        bool allowed = record.RequestCount < limit;
        if (allowed) { record.RequestCount++; record.UpdatedAt = DateTime.UtcNow; await _db.SaveChangesAsync(); }

        return (allowed, new RateLimitStatusResponse
        {
            RequestsUsed = record.RequestCount, RequestsLimit = limit,
            RequestsRemaining = Math.Max(0, limit - record.RequestCount),
            PercentUsed = Math.Round((double)record.RequestCount / limit * 100, 1),
            WindowResetAt = windowEnd, IsThrottled = !allowed
        });
    }
