...
public class InvoiceItemService : IInvoiceItemService
{
    ...
    public InvoiceItemService(ApplicationDbContext db, IMapper mapper) { _db = db; _mapper = mapper; }

    private async Task<(Invoice? invoice, bool hasAccess)> LoadInvoiceWithAccessCheckAsync(Guid invoiceId, Guid userId)
    {
        var invoice = await _db.Invoices
            .Include(i => i.Company).ThenInclude(c => c.CompanyUsers)
            .FirstOrDefaultAsync(i => i.Id == invoiceId);
        if (invoice == null) return (null, false);
        return (invoice, invoice.Company.CompanyUsers.Any(cu => cu.UserId == userId));
    }
        ...
        Guid invoiceId, AddInvoiceItemRequest request, Guid userId)
    {
        ...
        if (invoice == null) return ServiceResult<InvoiceItemResponse>.Fail("Invoice not found.", 404);
        if (!hasAccess) return ServiceResult<InvoiceItemResponse>.Fail("Access denied.", 403);
        ...
        var item = new InvoiceItem
        {
            ...
            LineTotal = Math.Round(request.Quantity * request.UnitPrice, 2),
        ...
        Guid itemId, UpdateInvoiceItemRequest request, Guid userId)
    {
        ...
        if (item == null) return ServiceResult<InvoiceItemResponse>.Fail("Item not found.", 404);

        var (_, hasAccess) = await LoadInvoiceWithAccessCheckAsync(item.InvoiceId, userId);
        if (!hasAccess) return ServiceResult<InvoiceItemResponse>.Fail("Access denied.", 403);
    ...
    {
        var item = await _db.InvoiceItems.FindAsync(itemId);
        if (item == null) return ServiceResult.Fail("Item not found.", 404);

        var (_, hasAccess) = await LoadInvoiceWithAccessCheckAsync(item.InvoiceId, userId);
        if (!hasAccess) return ServiceResult.Fail("Access denied.", 403);
    ...
    {
        var invoice = await _db.Invoices.Include(i => i.InvoiceItems).FirstOrDefaultAsync(i => i.Id == invoiceId);
        if (invoice == null) return;

        var items = invoice.InvoiceItems.ToList();
        invoice.SubTotal = items.Sum(i => i.LineTotal);
        invoice.TaxAmount = items.Sum(i => i.TaxRate.HasValue ? Math.Round(i.LineTotal * (i.TaxRate.Value / 100), 2) : 0);
