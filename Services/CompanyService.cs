...
public class CompanyService : ICompanyService
{
    ...
    public CompanyService(ApplicationDbContext db, IMapper mapper) { _db = db; _mapper = mapper; }
    ...
    public async Task<ServiceResult<CompanyResponse>> CreateCompanyAsync(CreateCompanyRequest request, Guid userId)
    {
        ...
        _db.CompanyUsers.Add(new CompanyUser { CompanyId = company.Id, UserId = userId, Role = "Owner" });
    ...
    public async Task<ServiceResult<List<CompanyResponse>>> GetMyCompaniesAsync(Guid userId)
    {
        ...
        var result = companyUsers.Select(cu => { var r = _mapper.Map<CompanyResponse>(cu.Company); r.UserRole = cu.Role; return r; }).ToList();
    ...
    public async Task<ServiceResult<CompanyResponse>> GetCompanyByIdAsync(Guid companyId, Guid userId)
    {
        ...
        if (cu == null) return ServiceResult<CompanyResponse>.Fail("Company not found or access denied.", 404);
