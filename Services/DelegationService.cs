...
public class DelegationService : IDelegationService
{
    ...
    public DelegationService(ApplicationDbContext db, INotificationService notifications, IEmailService email)
    {
        _db = db; _notifications = notifications; _email = email;
    }
        ...
        Guid companyId, CreateDelegationRequest request, Guid delegatorUserId)
    {
        ...
        if (!companyUserIds.Contains(delegatorUserId)) return ServiceResult<DelegationResponse>.Fail("Access denied.", 403);
        if (!companyUserIds.Contains(request.DelegateUserId)) return ServiceResult<DelegationResponse>.Fail("Delegate user is not a member of this company.", 400);
        if (request.DelegateUserId == delegatorUserId) return ServiceResult<DelegationResponse>.Fail("You cannot delegate to yourself.", 400);
        if (request.ActiveFrom >= request.ActiveUntil) return ServiceResult<DelegationResponse>.Fail("ActiveFrom must be before ActiveUntil.", 400);
        if (request.ActiveUntil < DateTime.UtcNow) return ServiceResult<DelegationResponse>.Fail("ActiveUntil must be in the future.", 400);

        bool overlaps = await _db.ApprovalDelegations.AnyAsync(d =>
            d.CompanyId == companyId && d.DelegatorUserId == delegatorUserId && d.IsActive &&
            d.ActiveFrom < request.ActiveUntil && d.ActiveUntil > request.ActiveFrom);
        if (overlaps) return ServiceResult<DelegationResponse>.Fail("An active delegation already overlaps with this period.", 409);
        ...
        {
            await _notifications.CreateAsync(request.DelegateUserId,
                $"{delegator.FullName} has delegated approvals to you",
                $"You will act on their behalf from {request.ActiveFrom:dd MMM} to {request.ActiveUntil:dd MMM yyyy}.", "Info");
        }
    ...
    public async Task<ServiceResult<List<DelegationResponse>>> GetForUserAsync(Guid userId, Guid companyId)
    {
        ...
        var ids = await _db.ApprovalDelegations
            .Where(d => d.CompanyId == companyId && (d.DelegatorUserId == userId || d.DelegateUserId == userId))
            .OrderByDescending(d => d.ActiveFrom)
            .Select(d => d.Id)
            .ToListAsync();

        var responses = new List<DelegationResponse>();
        foreach (var id in ids) responses.Add(await BuildResponseAsync(id));
    ...
    public async Task<ServiceResult<List<DelegationResponse>>> GetActiveForCompanyAsync(Guid companyId, Guid userId)
    {
        ...
        var ids = await _db.ApprovalDelegations
            .Where(d => d.CompanyId == companyId && d.IsActive && d.ActiveFrom <= now && d.ActiveUntil >= now)
    ...
    public async Task<ServiceResult> RevokeAsync(Guid delegationId, Guid userId)
    {
        ...
        bool canRevoke = delegation.DelegatorUserId == userId ||
            delegation.Company.CompanyUsers.Any(cu => cu.UserId == userId && (cu.Role == "Owner" || cu.Role == "Admin"));
        if (!canRevoke) return ServiceResult.Fail("Only the delegator or an admin can revoke a delegation.", 403);

        delegation.IsActive = false;
        delegation.ActiveUntil = DateTime.UtcNow;
        delegation.UpdatedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();
        return ServiceResult.Success(204);
    }

    public async Task<Guid> ResolveEffectiveApproverAsync(Guid originalUserId, Guid companyId, Guid? workflowTemplateId)
    {
        var now = DateTime.UtcNow;
        var delegation = await _db.ApprovalDelegations
            .Where(d => d.CompanyId == companyId && d.DelegatorUserId == originalUserId && d.IsActive &&
                        d.ActiveFrom <= now && d.ActiveUntil >= now &&
                        (d.LimitedToWorkflowTemplateId == null || d.LimitedToWorkflowTemplateId == workflowTemplateId))
            .FirstOrDefaultAsync();
